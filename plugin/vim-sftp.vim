...
function! SendAllFilesToRemoteForce()
	let command_line = GetVimSFTPCommandLine()
	execute command_line . 'send_all_files_to_remote_force'
endfunction
...
function! SendAllOpenBuffersToRemoteForce()
	let current_dir = fnamemodify(resolve(expand('<sfile>:p')), ':h')
	let buffers = getbufinfo()
	...
	echo items
	let command_line = GetVimSFTPCommandLine()
	execute command_line . 'send_multiple_files_to_remote_force ' . items
endfunction
